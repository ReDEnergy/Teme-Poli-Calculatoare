...
#define Lava		u_texture_1
#define TerrainN	u_texture_2
#define TerrainN2	u_texture_3
...
void main()
{
	vec4 diffuse_color = texture(Lava, texture_coord);
	vec4 Normal = texture(TerrainN, texture_coord);
	vec4 Normal2 = texture(TerrainN2, texture_coord);

	vec3 Normal3 = vec3(0.0, 1.0, 0.0);
	vec4 light_color = CalcSpotLight(world_position, Normal3.xyz);	
	frag_color =  diffuse_color + light_color;
}
