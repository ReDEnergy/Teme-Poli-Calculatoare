...
void Texture::create2DTextureFloat(const float* data, int width, int height, int bpp) {
	glGenTextures(1, &textureID);
	glBindTexture(GL_TEXTURE_2D, textureID);

	setParameters(GL_LINEAR, GL_LINEAR, GL_REPEAT);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA16F, width, height, 0, pixel_format[bpp], GL_FLOAT, (void*)data);
	CheckOpenGLError();
...
void Texture::createFrameBufferTexture(int width, int height, int target_id)
{
	...
	setParameters(GL_LINEAR, GL_LINEAR, GL_CLAMP_TO_EDGE);
	// ASK SOMEONE WHY GL_UNSIGNED_BYTE / GL_FLOAT
	glTexImage2D(textarget, 0, GL_RGBA16F, width, height, 0, GL_RGB, GL_FLOAT, 0);
