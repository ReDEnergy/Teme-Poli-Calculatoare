...
void build_word_sets(Domains& horizontal, Domains& vertical)
{
  ...
  for (unsigned int i = 0; i < rebus.rows; ++i) {
    for (unsigned int j = 0; j < rebus.columns; ++j) {
      if (rebus.is_empty(i, j) && (j == 0 || rebus.is_empty(i, j - 1) == false)) {
         ...
         * ar putea ipotetic sa fie completate incepand de acolo spre dreapta. */
         ...
         int lung = 0;
         int v=0;
        for (v=j; v<rebus.columns; v++) {
        	if (rebus.is_empty(i,v) == false) {
        		lung = v-j;
        		break;
        		}
        	 if (v == rebus.columns -1) 
        	 	lung = v-j+1;
        }
        
        vector<string> cuv;
        for (v=0; v<vocabular.size(); v++)
        	if (vocabular[v].size() == lung)
				cuv.push_back(vocabular[v]);
                
         horizontal.insert(std::pair< Position, std::vector<std::string> > (Position(i,j), cuv));
         
      }
      if (rebus.is_empty(i, j) && (i == 0 || rebus.is_empty(i - 1, j) == false)) {
        /* TODO: Puneti in vertical, la pozitia (i, j), toate cuvintele care ar
         * putea ipotetic sa fie completate incepand de acolo in jos. */
         
         int lung = 0;
         int v=0;
        for (v=i; v<rebus.rows; v++) {
        	if (rebus.is_empty(v,j) == false) {
        		lung = v-i;
        		break;
        		}
        	 if (v == rebus.rows - 1) 
        ...
        vertical.insert(std::pair< Position, std::vector<std::string> > (Position(i,j), cuv));      
      }
    }
  }
}
    ...
    std::vector<std::string>& verticalPrefixes)
{
  ...
  if (rebus.is_empty(row, col)) {
     ...
     * in dreapta. Puteti folosi functia putString din Rebus.h */
     ...
     rebus.putString(row,col,horizontal[Position(row,col)][0]);
     col +=horizontal[Position(row,col)][0].size();
     
	 if (col == rebus.columns)
     	  backtracking(row+1, 0, horizontal, vertical, verticalPrefixes);
	 if (col < rebus.columns)
     	  backtracking(row, col+1, horizontal, vertical, verticalPrefixes);       
